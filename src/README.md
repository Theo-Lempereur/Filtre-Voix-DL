...
| `denoiser.py` | **Boîte d'inférence** réutilisable `Denoiser` : durée libre (overlap-add 50 %), batch, détection `mask`/`complex`, contrat d'entrée 16 kHz mono. Source unique du forward d'inférence. |
...
box = Denoiser("checkpoints/rp_csm_final/best_si_sdri.pt")   # chemin souple : fichier, dossier ou nom de run
...
> autonome [`voice_denoiser`](../export/voice_denoiser/README.md).
